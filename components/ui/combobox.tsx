  ...
  return (
    <div className="relative not-has-[>*.w-full]:w-fit w-full has-disabled:opacity-64">
      ...
      <ComboboxPrimitive.Input
        ...
        data-slot="combobox-input"
        render={<Input className="rounded-full has-disabled:opacity-100" />}
        {...props}
      />
      {showTrigger && (
        <ComboboxTrigger
          className={cn(
            "-translate-y-1/2 absolute top-1/2 inline-flex size-8 shrink-0 cursor-pointer items-center justify-center rounded-full border border-transparent opacity-80 outline-none transition-opacity pointer-coarse:after:absolute pointer-coarse:after:min-h-11 pointer-coarse:after:min-w-11 hover:opacity-100 has-[+[data-slot=combobox-clear]]:hidden sm:size-7 [&_svg:not([class*='size-'])]:size-4.5 sm:[&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
...
function ComboboxChips({
  ...
  return (
    <ComboboxPrimitive.Chips
      className={cn(
        "relative inline-flex min-h-10 w-full flex-wrap gap-1 rounded-full border border-input bg-background px-3 py-1.5 text-sm shadow-xs outline-none ring-ring/24 transition-shadow focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2 ring-offset-background has-disabled:pointer-events-none has-disabled:opacity-50",
        className,
      )}
    ...
    >
      {startAddon && (
        <div
          aria-hidden="true"
          className="flex shrink-0 items-center pe-2 opacity-80 [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none"
          data-slot="combobox-start-addon"
        >
          {startAddon}
        </div>
...
function ComboboxChip({ children, ...props }: ComboboxPrimitive.Chip.Props) {
  return (
    <ComboboxPrimitive.Chip
      className="flex items-center rounded-full bg-accent px-2 py-0.5 font-medium text-accent-foreground text-sm outline-none [&_svg:not([class*='size-'])]:size-3.5"
